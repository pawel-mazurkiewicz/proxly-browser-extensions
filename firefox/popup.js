/**
 * Proxly Extension Popup JavaScript
...
class ProxlyPopup {
  ...
  async init() {
    console.log('Initializing Proxly popup');
  ...
  async loadSettings() {
    try {
      ...
      if (response && !response.error) {
        ...
        console.log('Settings loaded in popup:', this.settings);
      }
    } catch (error) {
  ...
  async handleModeChange(newMode) {
    try {
      ...
      // Save to storage
      await chrome.storage.sync.set({ linkMode: newMode });
